...
export default {
  ...
  computed: {
    canDelete() {
      return this.$auth.user.permissions.includes("subscriber.delete")
    }
  },
  watch: {
    options: {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        let { subscribers, pagination } = await this.$axios.$get("/admin/subscribers", config);
    ...
    deleteItem(id) {
      this.$axios.delete("/admin/subscribers/" + id)
      .then(() => {
        this.fetchData();
        this.$notifier.showMessage({
