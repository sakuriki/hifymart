          ...
          <v-tooltip bottom>
            <template #activator="{ on, attrs }">
              <v-btn
                ...
                color="success"
                icon
                :to="{ name: 'admin-taxes-id',params: { id: item.id }}"
                v-on="on"
              >
...
export default {
  ...
  computed: {
    canUpdate() {
      return this.$auth.user.permissions.includes("tax.update")
    },
    canDelete() {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        let { taxes, pagination } = await this.$axios.$get("/admin/taxes", config);
        this.data = taxes;
